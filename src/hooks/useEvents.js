...
export function useEvents() {
    ...
    const API_URL = 'http://localhost:5000/api/events'; // 백엔드 서버 주소

    // 1. 초기 로드 시 DB에서 데이터 불러오기 (GET)
    useEffect(() => {
        fetch(API_URL)
            .then(res => res.json())
            .then(data => {
     ...
     * UI가 버벅이지 않도록(Optimistic UI) React 화면을 먼저 바꾸고, 뒤에서 조용히 DB와 통신합니다.
     */
        ...
        setEvents(prev => {
            ...
            if (idx > -1) {
                if (mode === 'all') {
                    ...
                    const updatedEvent = {
                        ...data, startDate: originalEvent.startDate,
                        endDate: `${origEnd.getFullYear()}-${String(origEnd.getMonth() + 1).padStart(2, '0')}-${String(origEnd.getDate()).padStart(2, '0')}`,
                        excludedDates: originalEvent.excludedDates || [], repeatEndDate: originalEvent.repeatEndDate
                    };
                    next[idx] = updatedEvent;
                    saveToDB(updatedEvent, true); // DB 업데이트 요청 (PUT)
                    return next;
                    ...
                    saveToDB(next[idx], true); // 기존 일정 업데이트 (PUT)
                    saveToDB(newStandaloneEvent, false); // 새 단일 일정 생성 (POST)
                    return [...next, newStandaloneEvent];
                }
            }

            const newEvent = { ...data, excludedDates: [] };
            saveToDB(newEvent, false); // 새 일정 생성 (POST)
    ...
    const handleDeleteEvent = (eventId, instanceDate, mode = 'all') => {
        setEvents(prev => {
            ...
            if (mode === 'all' || !targetEvent.repeatUnit || targetEvent.repeatUnit === 'none') {
                fetch(`${API_URL}/${eventId}`, { method: 'DELETE' }).catch(err => console.error(err));
            ...
            if (mode === 'single') {
                ...
                saveToDB(next[targetIdx], true); // 예외 날짜 추가 후 DB 업데이트 (PUT)
                return next;
            }
            ...
            if (mode === 'future') {
                ...
                saveToDB(next[targetIdx], true); // 반복 종료일 단축 후 DB 업데이트 (PUT)
                return next;
            }
            return prev;
        });
    ...
    const handleUpdateEventDate = (eventId, newDate, newStartTime = null) => {
        ...
        setEvents(prev => prev.map(ev => {
            if (ev.id === eventId) {
                ...
                saveToDB(updatedEvent, true); // 드래그 앤 드롭 완료 후 DB 업데이트 (PUT)
                return updatedEvent;
            }
            return ev;
        }));
